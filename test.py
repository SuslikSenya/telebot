...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Студент":
        await update.message.reply_text("Слободенюк О.А.\nГрупа: ІО-24")
    elif text == "IT-технології":
        await update.message.reply_text(
            "Мови програмування: JavaScript, Python, Java, C++\n"
            "Операційні системи: Windows, Linux, MacOS\n"
            "Бази даних: MySQL, MongoDB\n"
            "ІDE: VSCode, PyCharm, Visual Studio, IntelliJ IDEA\n"
        )
    elif text == "Контакти":
        await update.message.reply_text(
...
async def handle_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        "contents": [
            {
                "role": "user",
                "parts": [
                    {"text": f"Надай користувачу коротку відповідь: {user_prompt}"}
                ]
            }
        ]
    }
    ...
    if response.status_code == 200:
        try:
            ...
            await update.message.reply_text(f"Результат: *{user_prompt}*:\n{answer}", parse_mode="Markdown")
